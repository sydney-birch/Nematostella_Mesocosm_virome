...
#Step 2: Run bowtie - loop through the list you just created - go into each dir and run the 
        #bowtie code and move back to the main dir and create 

def run_bowtie(ali_num,next_dir_name):
    os.mkdir("../{0}".format(next_dir_name))
    for dir_name in sample_list_result:
        os.chdir(dir_name)
        print ("cwd: ", os.getcwd())
        #Run hisat2
        result = subprocess.run("bowtie2 -q -p 12 --very-sensitive-local -x ../Nematostella_genome -1 *_paired1.fq -2 *_paired2.fq -S Nematostella_genome_BT_ali_{0}_{1}.sam".format(ali_num,dir_name), shell=True)
        print("bowtie complete on {0}".format(dir_name))
    
        print("cwd after bowtie b4 mv: ", os.getcwd())
        #make new dir for next round of mapping and move sam alignement file into the 2nd ali dir 
        os.mkdir("../../{0}/{1}".format(next_dir_name,dir_name))
        result = subprocess.run("mv Nematostella_genome_BT_ali_{0}_{1}.sam ../../{2}/{1}".format(ali_num,dir_name,next_dir_name), shell=True)
        #result = subprocess.run("mv unaligned_BT_{1}_ali{0}* ../../{2}/{1}".format(ali_num,dir_name,next_dir_name), shell=True)
        print("file move complete")
        os.chdir("..")
    print("All bowtie mapping is complete")

#call run_fastqc function
result_1 = run_bowtie(args.b, args.c)
