...
def run_samtools(ali_num):
    for dir_name in sample_list_result:
        ...
        samtools fastq -1 aligned_{0}_{1}_ali_paired1.fastq -2 aligned_{0}_{1}_ali_paired2.fastq -0 /dev/null -s /dev/null -n".format(dir_name, ali_num), shell=True)
        print("samtools complete on {0}".format(dir_name))
        #mv to RNA_filt dir for alignment
        result = subprocess.run("mv aligned_{0}_{1}_ali*.fastq ../../../4_RNA_filt/mapped_fastq_files".format(dir_name, ali_num), shell=True)
        os.chdir("..")
        print("cwd: ", os.getcwd())
