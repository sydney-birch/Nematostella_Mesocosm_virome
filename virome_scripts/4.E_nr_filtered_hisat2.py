...
def prep_work(output_dir,index_file):
    ...
    result = subprocess.run("hisat2-build {0} nr_db_index".format(index_file), shell=True)
...
def run_hisat2(input_dir,output_dir):
    ...
    for item in os.scandir():
        if "paired1" in item.name:
            ...
            #Run hisat2 
            result = subprocess.run("hisat2 -q -p 12 -x ../nr_db_index -1 filtered_unaligned_{0}_paired1.fq -2 filtered_unaligned_{0}_paired2.fq -S ../{1}/nr-filtered_{0}.sam".format(new_name, output_dir), shell=True)
        
#call function
hisat2_result = run_hisat2(args.c, args.a)

#./4.E_nr_filtered_hisat2.py -a ncbi_nr_filtered_unmapped_fastq_files -b ncbi_nr_DB.fa -c filtered_unmapped_fastq_files
