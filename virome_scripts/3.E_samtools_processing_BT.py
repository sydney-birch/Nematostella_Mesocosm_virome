...
def run_samtools(ali_num):
    for dir_name in sample_list_result:
        ...
        #run first part
        result = subprocess.run("samtools view Nematostella_genome_BT_ali_{0}_*.sam -f 0x4 -h -b -o unalign_reads.bam".format(ali_num), shell=True)
        #Run second part 
        result = subprocess.run("samtools collate -u -O unalign_reads.bam | \
        samtools fastq -1 unaligned_{0}_{1}_ali_paired1.fq -2 unaligned_{0}_{1}_ali_paired2.fq -0 /dev/null -s /dev/null -n".format(dir_name, ali_num), shell=True)
